...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = torch.nn.LSTMCell(embed_size + hidden_size,
                                         hidden_size=hidden_size,
                                         bias=True)
        self.h_projection = torch.nn.Linear(in_features=2*hidden_size,
                                            out_features=hidden_size,
                                            bias=False)
        self.c_projection = torch.nn.Linear(in_features=2*hidden_size,
                                            out_features=hidden_size,
                                            bias=False)
        self.att_projection = torch.nn.Linear(in_features=2*hidden_size,
                                              out_features=hidden_size,
                                              bias=False)
        self.combined_output_projection = torch.nn.Linear(in_features=3*hidden_size,
                                                          out_features=hidden_size,
                                                          bias=False)
        tgt_vocab_size = len(vocab.tgt)
        self.target_vocab_projection = torch.nn.Linear(in_features=hidden_size,
    ...
    def encode(
        ...
        #    using the source model embeddings.
        ...
        dec_init_state = (self.h_projection(init_decoder_hidden),
                          self.c_projection(init_decoder_cell))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # `enc_hiddens_proj`, which should be shape (b, src_len, h),
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        if enc_masks is not None:
            e_t.data.masked_fill_(
                enc_masks.byte().to(torch.bool), -float('inf'))
